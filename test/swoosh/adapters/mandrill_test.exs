defmodule Swoosh.Adapters.MandrillTest do
    ...
    Bypass.expect(bypass, fn conn ->
      ...
      body_params = %{
        ...
        "message" => %{
          ...
          "to" => [
            %{"type" => "bcc", "email" => "beast.avengers@example.com"},
            %{
              "type" => "bcc",
  ...
  test "deliver/1 with 2xx response containing errors", %{
    bypass: bypass,
    config: config,
    valid_email: email
  } do
    Bypass.expect(bypass, fn conn ->
      Plug.Conn.resp(
        conn,
        200,
        "[{\"email\":\"leafybasil.gmail@example.com\",\"status\":\"rejected\",\"_id\":\"e1f1f16d3c6e47c5955ad2b4c3207986\",\"reject_reason\":\"unsigned\"}]"
      )
    end)

    assert Mandrill.deliver(email, config) ==
             {:error,
              %{
                "_id" => "e1f1f16d3c6e47c5955ad2b4c3207986",
                "email" => "leafybasil.gmail@example.com",
  ...
  test "deliver/1 with template_name calls send-template endpoint", %{
    bypass: bypass,
    config: config
  } do
    email =
  ...
  test "deliver/1 with template_name but without template_content returns :ok", %{
    bypass: bypass,
    config: config
  } do
    email =
  ...
  test "deliver/1 with template_content and template_content returns :ok", %{
    bypass: bypass,
    config: config
  } do
    email =
      ...
      |> subject("Hello, Avengers!")
      |> put_provider_option(:template_name, "welcome")
      |> put_provider_option(:template_content, [
        %{"name" => "START_DATE", "content" => "Next Monday"}
      ])
